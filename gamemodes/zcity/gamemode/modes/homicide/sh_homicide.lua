...
Для тех, кто любит играть в шахматы.]],
	...
	["traitor_assasin"] = {
		Name = "Ассасин",
		PluvCoins = 900,
		Stats = { stamina = 300, recoilmul = 0.8 },
...
--\\Traitor Shop (PluvCoin)
MODE.ConVarName_Loadout = "hmcd_traitor_loadout"

if(CLIENT)then
	MODE.ConVar_Loadout = CreateClientConVar(MODE.ConVarName_Loadout, "", true, true, "Traitor shop loadout")
end
...
function MODE.GiveTraitorLoadout(ply, sub_role)
	...
	if role_info and role_info.Stats then
		if ply.organism then
			if role_info.Stats.stamina then ply.organism.stamina.max = role_info.Stats.stamina end
			if role_info.Stats.recoilmul then ply.organism.recoilmul = role_info.Stats.recoilmul end
		end
	end
	...
	local loadout = ply:GetInfo(MODE.ConVarName_Loadout) or ""
	local given = {}
	for _, class in ipairs(string.Explode(",", loadout)) do
