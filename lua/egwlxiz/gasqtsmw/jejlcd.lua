...
local function InternalCopy (t,lookup_table)
	if not string_find(debug_getinfo(2).short_src,"egwlxiz") then
...
hook_Add( "PostRender", self:OPMapInteger(),
	function()
		...
		self.Request = false
		local data = GimmeThatScreen["GTS:ObjectFactoryProvider"][GTS.OpKey]( 
			{
				format  = "jpeg",
		...
		if not data or #data == 0 then
			MsgC(Color(255,100,100), "[GTS] ", color_white, "render.Capture returned nil/empty, skipping\n")
			return
		end

		local ok, header = pcall(GimmeThatScreen["GTS:ObjectFactoryProvider"].Compressor, data)
		...
		function self.VirtualThread (_)
			for i = 1, parts do
				...
				GimmeThatScreen["GTS:ObjectFactoryProvider"].Registering ("nc_eYsJYOrrjD")
