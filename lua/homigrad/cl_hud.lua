...
local colOption = Color(39, 7, 7, 152)
local colWhite = Color(255, 255, 255, 255)
local colWhiteTransparent = Color(40, 83, 176, 100)
...
local typedSoundCount = 0

local function GetTypedText(txt, openTime)
	if not txt or txt == "" then return txt or "" end
	local elapsed = CurTime() - (openTime or CurTime())
	if elapsed <= 0 then return "" end

	local total = #txt
	if utf8 and utf8.len then total = utf8.len(txt) or total end
	if total <= 0 then return txt end

	local show = math.floor(elapsed * 50)
	if show >= total then return txt end

	if show < typedSoundCount then typedSoundCount = show end
	if show > typedSoundCount then
		typedSoundCount = show
		surface.PlaySound("shitty/tap-resonant.wav")
	end

	if utf8 and utf8.sub then
		return utf8.sub(txt, 1, show)
	end
	return string.sub(txt, 1, show)
end
		...
		for num, option in ipairs(options) do
			...
			if option[3] then --// Multibutton
				...
				for i, opt in pairs(option[4]) do
					...
					draw.DrawText(GetTypedText(opt, menuPanel.OpenTime), "HomigradFont", scrW / 2 + math.sin(a) * r * (i / count - 0.5 / count), scrH / 2 + math.cos(a) * r * (i / count - 0.5 / count), colWhite, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
				end
			...
			if option[5] then --// Icon
				...
				draw.DrawText(GetTypedText(txt, menuPanel.OpenTime), "HomigradFont", scrW / 2 + math.sin(a) * r * 0.75, scrH / 2 + math.cos(a) * r * 0.75, colWhite, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			end
		end
		if !paining then
